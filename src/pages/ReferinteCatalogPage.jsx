...
import HandZ from '../components/animation/HandZ';
import HandS2 from '../components/animation/HandS2';
import HandI2 from '../components/animation/HandI2';
...
/** Litere dinamice — animații 2D (pumn cu arătător scos, traseu specific fiecărei litere). */
const DYNAMIC_CATALOG_LETTERS = [
  { letter: 'Î', Component: HandI2 },
  { letter: 'J', Component: HandJ },
  { letter: 'Ș', Component: HandS2 },
  { letter: 'Ț', Component: HandT2 },
  { letter: 'X', Component: HandX },
  { letter: 'Z', Component: HandZ },
];
...
function DynamicLetterCard({ letter, Component }) {
  return (
      <div
          className="bg-black rounded-[22px] border border-ink-900/[0.06]
        shadow-card p-4 flex flex-col min-h-[220px]"
      >
        <div className="flex items-center justify-between mb-1">
        <span className="w-10 h-10 rounded-2xl bg-cream-100 text-ink-900 font-black text-xl
          flex items-center justify-center">
...
/**
 * Catalog de review — Faza 1: litere statice + litere dinamice.
      ...
      <div className="h-full bg-cream flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide px-4 sm:px-6 pb-10">
          <div className="max-w-5xl mx-auto">
            ...
            <h2 className="text-ink-900 font-bold text-sm tracking-[0.14em] mt-8 mb-3 px-1">
              LITERE DINAMICE
            </h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              {DYNAMIC_CATALOG_LETTERS.map(({ letter, Component }) => (
                  <DynamicLetterCard key={letter} letter={letter} Component={Component} />
              ))}
            </div>
          </div>
        </div>
